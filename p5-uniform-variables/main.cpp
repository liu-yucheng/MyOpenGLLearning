 ...
 *
 * References:
 * 1.   ogldev.org/www/tutorial05/tutorial05.html
 * 2.   glm.g-truc.net/0.9.9/api/modules.html
 * 
 * Intro:
 * A C++ (with OpenGL) program that shows a red triangle that shrinks and
 * stretches in size dynamically as time goes.
 * 
 * Note:
 * The scale of the triangle changes between -1.0 and 1.0.
...
static char const windowTitle[] = "Uniform Variables";
static GLuint vertexBuffer;
/* Reference of the uniform variable "scale" in the vertex shader text. */
static GLuint scale;
...
/* Displays the objects to be drawn. */
static void display() {
...
static void loadShaderProgram() {
    ...
    // Locate the uniform variable "scale" in the shader program
    scale = glGetUniformLocation(shaderProgram, "scale");
    if (scale == 0xFFFFFFFF) {
        errShowLine(funcName, "error: locating uniform variable \"scale\"");
        exit(1);
    }
}
